...
class euler:
	...
	def pad(self, u, left_pad, right_pad):
		...
		if left_pad >= 0 and right_pad >= 0:
			...
			# print(np.shape(u[:self.k]))
			# print(np.shape(u[1:self.k]))
			return u[1:self.k]
		elif left_pad >= 0 and right_pad < 0:
			return u[0:self.k-1]
	...
	def weno_characteristic_left(self):
		...
		for k_s in range(self.r+1): # for each stencil
			...
			u_p_reconstructed = u_p_reconstructed + omega[k_s]*P[k_s]
		# print(np.shape((np.matmul(Q, u_p_reconstructed.reshape(len(u),self.num_vars,1))).reshape((len(u),self.num_vars))[self.r:self.k+self.r-1]))
		# print(np.shape((np.matmul(Q, u_p_reconstructed.reshape(len(u),self.num_vars,1))).reshape((len(u),self.num_vars))[self.r:-self.r]))
		return (np.matmul(Q, u_p_reconstructed.reshape(len(u),self.num_vars,1))).reshape((len(u),self.num_vars))[self.r:self.k+self.r-1]
	...
	def weno_characteristic_right(self):
		...
		for k_s in range(self.r+1):
			for l in range(self.r+1):
				P[k_s,:,:] += self.a[k_s,l]*np.matmul(Q_inverse, shift(u, k_s-l).reshape((len(u), self.num_vars, 1))).reshape((len(u), self.num_vars))
			u_p_reconstructed = u_p_reconstructed + omega[k_s]*P[k_s]
		return np.flip((np.matmul(Q, u_p_reconstructed.reshape(len(u),self.num_vars,1))).reshape((len(u),self.num_vars)), axis=0)[self.r:self.k+self.r-1]
	...
	def flux_split(self):
		...
		# print(np.shape(u_p_reconstructed_l))
	...
	def get_dudx(self):
		# print(self.flux_split())
		u_split = self.pad(self.flux_split(), self.r+1, self.r+1)
	
		dudx = np.zeros(np.shape(u_split))
	
		for i in range(len(self.d)):
			dudx += self.d[i]*(shift(u_split, -i)-shift(u_split, i+1))
		end_position = self.k+self.r+1
...
rk4 = euler(x0, xf, tf, N, 0.2, 1, None, characteristic, 'edge', 'rk4', problem_type, False)
