...
class euler:
	...
	def recursive_order_reduction(self):
		...
		# Extend original array, providing two extra points on the right boundary to account for (1) shift in characteristic 
		# transform calulation, and (2) additional required half point for calculating ROR at last cell
		u_p_extended = np.pad(self.u_p, ((self.r, self.r+2), (0,0)), mode='reflect', reflect_type='odd')
		u_left = u_p_extended[0:-1]
		u_right = np.flip(u_p_extended[1:], axis=0)

		Q, Q_inverse = self.get_characteristic_transform(u_p_extended)

		u_p_extended = u_p_extended[:-1] # after we calculate the characteristics, remove the right-most cell

		while ROR == False:
			u_p_reconstructed_l, u_p_reconstructed_r = self.weno_characteristic(u_p_extended, Q, Q_inverse, r)

			rho_diff_half_point_l = np.abs(self.u_p[:,0] - u_p_reconstructed_l[:,0])
			p_diff_half_point_l = np.abs(self.u_p[:,2] - u_p_reconstructed_l[:,2])

			rho_diff_half_point_r = np.abs(u_left[self.r+1:-self.r+1][:,0] - u_p_reconstructed_r[:,0])
			p_diff_half_point_r = np.abs(u_left[self.r+1:-self.r+1][:,2] - u_p_reconstructed_r[:,2])

			rho_max_diff_array = np.empty(2*self.r-1)
			p_max_diff_array = np.empty(2*self.r-1)
			l_array = np.arange(-self.r+1, self.r)
			
			for idx, l in enumerate(l_array):		
				rho_max_diff_array[idx] = np.max((np.abs(shift(u_left[:,0], -l-1) - shift(u_left[:,0], -l)))[self.r:-self.r])
				p_max_diff_array[idx] = np.max((np.abs(shift(u_left[:,2], -l-1) - shift(u_left[:,2], -l)))[self.r:-self.r])
			
			ROR_l = (rho_diff_half_point_l <= 1/2*np.max(rho_max_diff_array[0:-1])) & (p_diff_half_point_l <= 1/2*np.max(p_max_diff_array[0:-1]))
			ROR_r = (rho_diff_half_point_r <= 1/2*np.max(rho_max_diff_array[1:])) & (p_diff_half_point_r <= 1/2*np.max(p_max_diff_array[1:]))

			if np.any(ROR_l == False) or np.any(ROR_r == False):
				# isolate the false cells
				r = r - 1
				# print(rho_diff_half_point_l)
				# print(rho_max_diff_array)
				# print(ROR_l)
				# print(self.t)
				# plt.plot(self.x, self.u_p[:,0], marker='o')
				# plt.plot(np.linspace(self.x_0 + self.h / 2, self.x_f+ self.h / 2, self.k), u_p_reconstructed_l[:,0], marker='.')
				# plt.plot(np.linspace(self.x_0 + self.h / 2, self.x_f+ self.h / 2, self.k), u_p_reconstructed_r[:,0], marker='^')
				# plt.show()
				# sys.exit()
			else:
				ROR = True
		return u_p_reconstructed_l_final, u_p_reconstructed_r_final

	def weno_characteristic(self, u_p_extended, Q, Q_inverse, r):
		# print(self.u_p[:,0])
		# u_left = np.pad(self.u_p, ((self.r, self.r), (0,0)), mode='reflect', reflect_type='odd')
		u_left = u_p_extended[0:-1]
		...
		sys.exit()
		beta_left = calculate_beta_characteristic(u_left, self.r, Q, Q_inverse, shift)

		# lambda_left = np.nanmin(beta_left)/(self.epsilon_m + np.nanmax(beta_left))
	
		# u_right = np.flip(np.pad(self.u_p, ((self.r-1, self.r+1), (0,0)), mode='reflect', reflect_type='odd'), axis=0)
		# u_right = np.flip(u_p_extended[1:])
		...
		for k_s in range(self.r+1): # for each stencil
			...
			u_p_reconstructed_left = u_p_reconstructed_left + omega_left[k_s]*P_left[k_s]
			u_p_reconstructed_right = u_p_reconstructed_right + omega_right[k_s]*P_right[k_s]
		...
		# u_p_reconstructed_l = (np.matmul(Q_left, u_p_reconstructed_left.reshape(len(u_left),self.num_vars,1))).reshape((len(u_left),self.num_vars)) # just remove 1 point for the reconstruction algorithm
		...
		# u_p_reconstructed_r = np.flip((np.matmul(Q_right, u_p_reconstructed_right.reshape(len(u_right),self.num_vars,1))).reshape((len(u_right),self.num_vars)), axis=0)
		...
		# print(u_left[:,0])
		# print("")	
		# print(u_p_reconstructed_l[:,0])
		# print(np.flip(u_right[:,0]))
		# print(u_p_reconstructed_r[:,0])
		rho_diff_half_point_l = np.abs(self.u_p[:,0] - u_p_reconstructed_l[:,0])
		p_diff_half_point_l = np.abs(self.u_p[:,2] - u_p_reconstructed_l[:,2])
		...
		# print(self.u_p)
		# print(u_left[self.r+1:-self.r+1])
		# sys.exit()
		# print(rho_diff_half_point_l)
		# print(rho_diff_half_point_r)
		# sys.exit()
		# print(u_left[:,0])
		# print(p_diff_half_point_r)
		# print(p_diff_half_point_l)

		# print("")
		# print(rho_diff_half_point_r)
		# print(u_p_reconstructed_l[:,0] - u_left[:,0])
		# sys.exit()

		rho_max_diff_array = np.empty(2*self.r-1)
		p_max_diff_array = np.empty(2*self.r-1)
		# print(rho_diff_array)
		l_array = np.arange(-self.r+1, self.r)
		# print(u_left[:,0])
		# print(self.u_p[:,0])
		for idx, l in enumerate(l_array):		
		...
		# print(rho_max_diff_array, p_max_diff_array)
		# sys.exit()
		

		# print(rho_diff_half_point_l <= 1/2*np.max(rho_max_diff_array[0:-1]))
		# print(p_diff_half_point_l <= 1/2*np.max(p_max_diff_array[0:-1]))
		# print(rho_diff_half_point_r <= 1/2*np.max(rho_max_diff_array[1:]))
		# print(p_diff_half_point_r <= 1/2*np.max(p_max_diff_array[1:]))
		...
		if np.any(ROR_l == False):
			...
			plt.show()
			sys.exit()
		if np.any(ROR_r == False):
			print("right reconstruction reduction")
			print(self.t)
		# sys.exit()
		# print(np.all(rho_diff_half_point_l <= 1/2*np.max(rho_max_diff_array[0:-1])))
		# print(np.all(p_diff_half_point_l <= 1/2*np.max(p_max_diff_array[0:-1])))
		# print(np.all(rho_diff_half_point_r <= 1/2*np.max(rho_max_diff_array[1:])))
		# print(np.all(p_diff_half_point_r <= 1/2*np.max(p_max_diff_array[1:])))

		return u_p_reconstructed_l[0:-1], u_p_reconstructed_r[0:-1]
		# print(rho_diff_half_point_l)
		# print(1/2*np.max(rho_max_diff_array[1:]))
		# print(rho_diff_half_point_l - 1/2*np.max(rho_max_diff_array[1:]))
		# sys.exit()
	...
	def flux_split(self):
		...
		u_p_reconstructed_l, u_p_reconstructed_r = self.recursive_order_reduction()
...
a = euler(x0, xf, tf, 5, 0.6, 2, 4, characteristic, 'ssprk3')
