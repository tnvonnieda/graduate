...
class euler:
	...
	def get_temperature_from_primitive(self, u_p):
		return (( (self.gamma_1*self.Pi_1+u_p[:,2])*(self.gamma_2*self.Pi_2+u_p[:,2]) ) / ( u_p[:,0] * ( u_p[:,3]*(self.gamma_1-1)*(self.gamma_2*self.Pi_2+u_p[:,2])*self.c_v_1 + (1-u_p[:,3])*(self.gamma_2-1)*(self.gamma_1*self.Pi_1+u_p[:,2])*self.c_v_2 ) ))
